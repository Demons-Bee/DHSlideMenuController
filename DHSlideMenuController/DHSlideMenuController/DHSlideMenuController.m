...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    if (self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil]) {
        _leftViewShowWidth=150;
        _rightViewShowWidth=150;
...
- (void)layoutCurrentViewWithOffset:(CGFloat)xOffset {
    ...
    if (xOffset > 0) {//向右滑的
        [currentView setFrame:CGRectMake(xOffset, baseView.bounds.origin.y + (totalHeight * (1 - scale) / 2), totalWidth * scale, totalHeight * scale)];
    } else {//向左滑的
        [currentView setFrame:CGRectMake(baseView.frame.size.width * (1 - scale) + xOffset, baseView.bounds.origin.y + (totalHeight * (1 - scale) / 2), totalWidth * scale, totalHeight * scale)];
    }
}
